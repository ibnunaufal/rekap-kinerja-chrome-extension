...
//   const url = chrome.runtime.getURL("assets/e-kinerja-logo.png");
...
async function captureAndDownload(format, currentUrl) {
  console.log("Current URL inside captureAndDownload:", currentUrl);
  if (!currentUrl.includes("kinerja.bkn.go.id/kinerja_harian")) {
    alert("Silakan buka halaman eKinerja pada tab Mingguan terlebih dahulu.");
    return;
  }
          ...
          children: [
            ...
            new Paragraph({
              children: [
                ...
                new TextRun({
                  text: `${weekDates[0].replaceAll("-", "/")} - ${weekDates[
                    weekDates.length - 1
                  ].replaceAll("-", "/")}`,
                  size: 24,
                  font: "Arial",
